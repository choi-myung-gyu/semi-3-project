...
public class TempServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		// 1. TempDAO 를 사용하여 전체 데이터 조회
		TempDAO dao = new TempDAO();
		
		// 2. ArrayList<TempVO> 에 전체 조회 데이터 정보를 저장
		ArrayList<TempVO> datas = dao.getAll();
		
		// 3. request.setAttribute() 이용하여 웹 페이지에 조회한 데이터를 테이블 구조로
		// 만들 수 있는 JSP 에 데이터 전달
		request.setAttribute("datas", datas);
		
		// 4. RequestDispatcher 를 사용하여 조회한 데이터를 테이블 구조로 만들 JSP 파일로
		// 포워드 수행
